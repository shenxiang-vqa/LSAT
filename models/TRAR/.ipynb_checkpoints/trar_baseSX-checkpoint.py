...
# ------------------------------------
# ---- Multi-Head Attention image ----
# ------------------------------------

class IMAGE_MHAtt(nn.Module):
    def __init__(self, __C):
        super(IMAGE_MHAtt, self).__init__()
        self.__C = __C
...
#         ).transpose(1, 2)
        ...
        attn = LocalAttention(dim=64, window_size=32, causal=False, look_backward=1, look_forward=1, dropout=0.1, exact_windowsize=False)
        atted = attn(v, k, q, mask)
        atted = atted.transpose(1, 2).contiguous().view(
            n_batches,
            -1,
            self.__C.HIDDEN_SIZE
